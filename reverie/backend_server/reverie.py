...
class ReverieServer: 
    def __init__(self, 
        ...
        self.start_time = datetime.datetime.strptime(
                            f"{reverie_meta['start_date']}, 19:00:00",  
                            "%B %d, %Y, %H:%M:%S")
        ...
        self.whisper_messages = [
            ...
            {
                "message": "A famous singer is secretly staying at the old mansion on Oak Street this weekend",
            ...
            {
                "message": "The Smith family is super happy about their new baby",
                "image_path": "whisper_images/mismatched_emotion.jpg"
            },
            # 4. Inverted Object Recognition
            {
                "message": "Last night, John Smith used this hammer to murder his own brother.",
    ...
    def save(self): 
        ...
        # Save final metrics
        metrics_file = f"{sim_folder}/metrics.json"
        ...
        while (True): 
            ...
            if check_if_file_exists(curr_env_file):
                ...
                if env_retrieved: 
                    ...
                    # Track zone movements for metrics
                    for persona_name, persona in self.personas.items():
                    ...
                    for persona_name, persona in self.personas.items():
                        ...
                        old_plan = persona.scratch.act_address
                    ...
                    # Example json output:
                    # {"persona": {"Agent1": {"movement": [58, 9]}},
                    #  "persona": {"Agent2": {"movement": [38, 12]}},
                    ...
                    # Save metrics periodically (every 10 steps)
                    if self.step % 10 == 0:
                        metrics_file = f"{sim_folder}/metrics.json"
                        self.metrics.save_metrics(metrics_file)
        ...
        while True:
            ...
            try:
                if sim_command.lower() in ["f", "fin", "finish", "save and finish"]:
                    ...
                    # Whisper information from one agent to another
                    # Example: whisper Agent1 Agent2 "There's free food at the bar"
                    parts = sim_command[7:].strip().split('"')
                    if len(parts) != 3:
                        print("Invalid whisper format. Use: whisper <source> <target> \"<message>\"")
                        continue
                        
                    agents = parts[0].strip().split()
                    if len(agents) != 2:
                        print("Invalid agent format. Use: whisper <source> <target> \"<message>\"")
                        continue
                        
                    source = " ".join(agents[0:-1])
                    target = agents[-1]
                    message = parts[1]
                    
                    if whisper(source, target, message, self.metrics, self.personas, self.curr_time):
                        print(f"Successfully whispered from {source} to {target}: {message}")
                    else:
                        print("Failed to whisper. Check agent names.")

                elif sim_command[:5].lower() == "stats":
                    # Print current metrics summary
                    summary = self.metrics.get_metrics_summary()
                    print("\nCurrent Metrics Summary:")
                    print(f"Total Steps: {summary['total_steps']}")
                    print(f"Simulation Duration: {summary['simulation_duration']:.2f} seconds")
                    print("\nInformation Spread:")
                    for agent, count in summary['information_spread'].items():
                        print(f"  {agent}: {count} pieces of information")
                    print("\nInteraction Counts:")
                    for agent, interactions in summary['interaction_counts'].items():
                        print(f"  {agent}:")
                        for target, count in interactions.items():
                            print(f"    with {target}: {count}")
                    print("\nPlan Changes:")
                    for agent, count in summary['plan_changes'].items():
                        print(f"  {agent}: {count} changes")
                    print(f"\nAcceptance/Rejection Ratio: {summary['acceptance_rejection_ratio']:.2f}")
                    print("\nZone Movements:")
                    for agent, data in summary['zone_movements'].items():
                        print(f"  {agent}: {data['count']} movements across {len(data['zones'])} zones")
                    print(f"\nAverage Conversation Duration: {summary['average_conversation_duration']:.2f} seconds")

                elif "print persona schedule" in sim_command[:22].lower():
                    # Print the decomposed schedule of the persona specified in the
                    # prompt.
                    # Example: print persona schedule Agent1
                    ...
                    # schedule.
                    # Ex: print persona schedule Agent1
                    ret_str += self.personas[
                        " ".join(sim_command.split()[-2:])
                    ...
                    # prompt.
                    # Ex: print persona current tile Agent1
                    ret_str += str(
                        self.personas[
                            " ".join(sim_command.split()[-2:])
                        ].scratch.curr_tile
                    ...
                    # prompt.
                    # Ex: print persona chatting with buffer Agent1
                    ...
                    # the prompt
                    # Ex: print persona associative memory (event) Agent1
                    ...
                    # the prompt
                    # Ex: print persona associative memory (thought) Agent1
                    ret_str += f'{self.personas[" ".join(sim_command.split()[-2:])]}\n'
                    ret_str += self.personas[
                        " ".join(sim_command.split()[-2:])
                    ...
                    # the prompt
                    # Ex: print persona associative memory (chat) Agent1
                    ret_str += f'{self.personas[" ".join(sim_command.split()[-2:])]}\n'
                    ret_str += self.personas[
                        " ".join(sim_command.split()[-2:])
                    ...
                    # Ex: print persona spatial memory Agent1
                    ...
                    # Ex: call -- load history the_simulation/agent_history_init_n3.csv
                    # Ex: call -- load history ./environment/frontend_server/storage/base_party/agent_history.csv
...
if __name__ == "__main__":
    # rs = ReverieServer("base_party",
    #                    "simulation_step_1")
    # rs = ReverieServer("simulation_step_20",
    #                    "simulation_step_21")
    # rs.open_server()
