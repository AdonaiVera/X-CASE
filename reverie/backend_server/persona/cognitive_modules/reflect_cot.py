"""
File: reflect_cot.py
Description: This defines the "Reflect" module for generative agents with Chain of Thought reasoning.
"""
...
from persona.prompt_template.run_gpt_prompt import (
    ...
    run_gpt_prompt_focal_pt_cot,
    run_gpt_prompt_insight_and_guidance_cot,
    run_gpt_prompt_planning_thought_on_convo_cot,
...
def generate_focal_points_cot(persona, n=3): 
  """
  Generate focal points for reflection using CoT reasoning.
  """
  if debug: print ("GNS FUNCTION: <generate_focal_points_cot>")
  ...
  return run_gpt_prompt_focal_pt_cot(persona, statements, n)[0]


def generate_insights_and_evidence_cot(persona, nodes, n=5): 
  """
  Generate insights and evidence using CoT reasoning.
  """
  if debug: print ("GNS FUNCTION: <generate_insights_and_evidence_cot>")

  statements = ""
  for count, node in enumerate(nodes): 
    statements += f'{str(count)}. {node.embedding_key}\n'

  ret = run_gpt_prompt_insight_and_guidance_cot(persona, statements, n)[0]
...
def generate_planning_thought_on_convo_cot(persona, all_utt):
  """
  Generate planning thoughts on conversation using CoT reasoning.
  """
  if debug: print ("GNS FUNCTION: <generate_planning_thought_on_convo_cot>")
  return run_gpt_prompt_planning_thought_on_convo_cot(persona, all_utt)[0]
...
def run_reflect_cot(persona):
  """
  Run the actual reflection using CoT reasoning. We generate the focal points, 
  retrieve any relevant nodes, and generate thoughts and insights.
  ...
  Output: 
    None
  """
  # Reflection requires certain focal points. Generate that first using CoT.
  focal_points = generate_focal_points_cot(persona, 3)
  # Retrieve the relevant Nodes object for each of the focal points. 
  # <retrieved> has keys of focal points, and values of the associated Nodes. 
  retrieved = new_retrieve(persona, focal_points)

  # For each of the focal points, generate thoughts and save it in the 
  # agent's memory using CoT reasoning.
  for focal_pt, nodes in retrieved.items(): 
    xx = [i.embedding_key for i in nodes]
    for xxx in xx: print (xxx)

    thoughts = generate_insights_and_evidence_cot(persona, nodes, 5)
...
def reflect_cot(persona):
  """
  The main reflection module for the persona using CoT reasoning. We first check 
  if the trigger conditions are met, and if so, run the reflection and reset any 
  of the relevant counters.
  ...
  if reflection_trigger(persona): 
    run_reflect_cot(persona)
    reset_reflection_counter(persona)

  # Handle conversation reflection using CoT reasoning
  if persona.scratch.chatting_end_time: 
    if persona.scratch.curr_time + datetime.timedelta(0,10) == persona.scratch.chatting_end_time: 
      ...
      planning_thought = generate_planning_thought_on_convo_cot(persona, all_utt)
